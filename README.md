...
[![Docker Image](https://ghcr.io/badges/your-org/wiki-llm.svg)](https://github.com/your-org/wiki-llm/pkgs/container/wiki-llm)
...
```bash
pip install git+https://github.com/your-org/wiki-llm.git
```

### Docker (recommended for teams)

```bash
docker pull ghcr.io/your-org/wiki-llm:latest

docker run --rm \
  --env-file .env \
  -v $(pwd)/content_new:/app/content_new \
  -v $(pwd)/content_processed:/app/content_processed \
  -v $(pwd)/content_error:/app/content_error \
  -v $(pwd)/wiki:/app/wiki \
  -v $(pwd)/config:/app/config \
  ghcr.io/your-org/wiki-llm:latest \
  python -m src --config config/my_config.py run-all
```

### Kubernetes (production)

To run the pipeline as a `CronJob` and the UI as a `Deployment`:

```bash
kubectl apply -f k8s/
```

See [`k8s/`](#kubernetes) for the full manifests.
...
**3. Create your config** (or use the included example):
...
docker build -t wiki-llm:local .
```

### GitHub Actions -> GHCR

The `.github/workflows/ci.yml` workflow automatically builds and pushes to `ghcr.io/your-org/wiki-llm` on every push to `main`.
...
- name: Build and push
  ...
  with:
    push: true
    tags: ghcr.io/your-org/wiki-llm:${{ github.sha }}
```
...
services:
  pipeline:
    image: ghcr.io/your-org/wiki-llm:latest
    env_file: .env
    volumes:
      - ./content_new:/app/content_new
      - ./content_processed:/app/content_processed
      - ./content_error:/app/content_error
      - ./wiki:/app/wiki
      - ./config:/app/config
    command: python -m src --config config/my_wiki.py run-all
  ...
  chat:
    ...
    volumes:
      - ./wiki:/app/wiki
      - ./config:/app/config
    command: python -m src --config config/my_wiki.py chat
```
...
For teams already using Kubernetes, manifests are provided in `k8s/`:

| File | Description |
| --- | --- |
| `k8s/pipeline-cronjob.yaml` | CronJob that runs `run-all` on a configurable schedule |
| `k8s/chat-deployment.yaml` | Chat server Deployment with a NodePort Service |
| `k8s/configmap.yaml` | Non-secret environment variables |
| `k8s/secret.yaml` | Template for creating the API-key Secret |

```bash
# Create the secret with API keys
kubectl create secret generic wiki-llm-secrets \
  --from-literal=OPENROUTER_APIKEY=sk-or-... \
  --from-literal=WIKI_BACKEND=openrouter

# Apply everything
kubectl apply -f k8s/

# Trigger the pipeline manually
kubectl create job wiki-run --from=cronjob/wiki-llm-pipeline
```

> **Tip:** mount a `PersistentVolumeClaim` for `/app/wiki` and `/app/content_new` so data survives pod restarts.
...
2. `pip install -e ".[dev]"` — install development dependencies
