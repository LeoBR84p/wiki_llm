...
Executes up to eight stages in order:
  ...
  3. key_themes  — collect terms + generate key theme pages
  4. groups      — generate organizational group pages
...
from .stages.key_theme import run_key_themes
...
class PipelineOptions:
    ...
    stages: list[str] = field(default_factory=lambda: [
        "read", "generate", "key_themes", "groups", "index",
...
async def run_pipeline(cfg: WikiConfig, opts: PipelineOptions | None = None) -> PipelineResult:
    ...
    Args:
        cfg: Active WikiConfig describing content dirs, wiki dir, LLM settings,
            objects, key themes, groups, and prompt paths.
    ...
    with progress:
        ...
        # 3. Key Themes
        # ------------------------------------------------------------------
        if "key_themes" in stages and cfg.key_themes:
            t_tax = progress.add_task("key themes  · normalizing terms…", total=None)
            await run_key_themes(cfg, llm, llm_logger)
            progress.update(t_tax, description="key themes  · done", total=1, completed=1)

        # ------------------------------------------------------------------
        # 4. Groups
        # ------------------------------------------------------------------
        if "groups" in stages and cfg.groups:
            t_grp = progress.add_task("groups      · building group pages…", total=None)
        ...
        # 9. Export (Word)
        # ------------------------------------------------------------------
        if "export" in stages and cfg.export_word:
...
def _export_word(cfg: WikiConfig) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            "markdown_hero.word_format is not available — install it or set export_word=False"
        ) from exc
