...
For each GroupConfig, groups documents by the value of metadata.extra[metadata_field]
and generates one summary page per distinct value under wiki_dir/group.wiki_subdir/.
"""
...
from ..models.config import GroupConfig, WikiConfig
from ..models.document import Document
from ._utils import write_atomic, _safe_slug
...
def _page_content(group_name: str, field: str, docs: list[Document], grp_cfg: GroupConfig) -> str:
    """Build the Markdown content for a group summary page.
    ...
    Args:
        ...
        grp_cfg: GroupConfig that provides the section name.
...
async def _generate_group_page_llm(
    ...
    """Generate a group summary page via LLM.

    Args:
        group_name: The distinct metadata value defining this group.
        grp_cfg: GroupConfig providing the prompt path.
    ...
    Returns:
        Markdown content string for the group page.
    """
    ...
    context = {
        ...
        "group_config_name": grp_cfg.name,
        "docs": docs_list,
        "total": len(docs),
...
async def run_groups(
    ...
    """Generate group pages for all configured GroupConfig entries.

    For each group, partitions the provided documents by their metadata
    ...
    Args:
        cfg: Active WikiConfig with wiki_dir and groups list.
    ...
    for grp_cfg in cfg.groups:
        logger.info("Group: %s (field: %s)", grp_cfg.name, grp_cfg.metadata_field)
        partitions: dict[str, list[Document]] = {}
        for doc in docs:
            val = _group_value(doc, grp_cfg.metadata_field)
            if val:
                partitions.setdefault(val, []).append(doc)

        grp_dir = cfg.wiki_dir / grp_cfg.wiki_subdir
        for group_name, group_docs in partitions.items():
            ...
            logger.info("  Group page generated: %s", dest.name)
