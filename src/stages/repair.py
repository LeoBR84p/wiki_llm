...
async def _add_link_to_page(
    wiki_dir: Path, page_id: str, link_id: str, file_locks: dict[str, asyncio.Lock]
) -> bool:
...
class RepairGlobalState(TypedDict):
    ...
    Attributes:
        ...
        broken_links: List of {source, target} dicts from static_analysis.
...
def _build_item_states(state: RepairGlobalState) -> list[dict[str, Any]]:
    ...
    for bl in state["broken_links"]:
        by_dest.setdefault(bl["target"], []).append(bl["source"])
...
async def _repair_broken_link(
    ...
    async with creating_lock:
        if slug in creating_pages or dest_path.exists():
            return [], []
        creating_pages.add(slug)
    ...
    try:
        ...
        for sid in suggested[:5]:
            if sid in {c.lower() for c in candidate_ids}:
                if await _add_link_to_page(wiki_dir, sid, target, file_locks):
                    added.append(sid)
    ...
    async def repair_item(item: RepairItemState) -> dict[str, Any]:
        ...
        if item["repair_type"] == "broken_link":
            repaired, errors = await _repair_broken_link(
                wdir, item["target"], item["sources"], cfg, llm, llm_logger,
                creating_pages, creating_lock, file_locks,
            )
        else:
            repaired, errors = await _repair_orphan(
                wdir, item["target"], cfg, llm, llm_logger, file_locks,
            )
