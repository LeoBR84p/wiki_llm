...
# ---------------------------------------------------------------------------
# Object types, key themes and groups
# ---------------------------------------------------------------------------


class ObjectTypeConfig(BaseModel):
    """Defines one object type that the wiki is built around.

    An object type is the central document type the wiki exists to capture.
    Each object type gets its own wiki subdirectory and its own generate/evaluate prompts.

    Example object types: Policy, Report, Article, Product Specification.
    ...
    Attributes:
        ...
        wiki_subdir: Subdirectory name under wiki_dir for this object type.
...
class KeyThemeConfig(BaseModel):
    """Defines a key theme dimension (e.g. Topics, Tags, Domains).

    Key themes are topic dimensions automatically extracted from generated wiki pages.
    After each object page is written, the pipeline scans it for theme terms and
    generates one summary page per canonical term.

    Attributes:
        name: Human-readable theme name (e.g. "Topics").
        wiki_subdir: Subdirectory under wiki_dir for key theme pages.
        ...
        prompt_create_page: Prompt file for generating key theme summary pages.
    """
    ...
    def _check_source_fields(self) -> "KeyThemeConfig":
...
class GroupConfig(BaseModel):
    """Defines an organizational group dimension (e.g. Teams, Departments, Authors).

    Groups organize the wiki by a metadata field that already exists in source
    documents.  Each distinct value in that field becomes a group page — a simple
    index of all objects that share that value.  No AI is involved.

    Attributes:
        name: Human-readable group name (e.g. "Team").
        wiki_subdir: Subdirectory under wiki_dir for group pages.
...
class WikiConfig(BaseModel):
    ...
    Attributes:
        ...
        objects: Ordered list of object type definitions (must be non-empty).
        key_themes: Optional key theme dimensions.
        groups: Optional organizational group dimensions.
    ...
    objects: list[ObjectTypeConfig]
    key_themes: list[KeyThemeConfig] = Field(default_factory=list)
    ...
    @model_validator(mode="after")
    def _check_objects(self) -> WikiConfig:
        if not self.objects:
            raise ValueError("objects cannot be empty.")
        slugs = [e.slug for e in self.objects]
        if len(slugs) != len(set(slugs)):
            raise ValueError("objects slugs must be unique.")
        return self

    def object_by_slug(self, slug: str) -> ObjectTypeConfig | None:
